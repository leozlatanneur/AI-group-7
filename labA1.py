# Group 7
#members: 
#136339	Ngandu Given Kaseba
#136411	Thang'wa Anthony Mwenda
#131440	Njogu Jeff Mwangi
#136201	Mutiki Kizito
#122732	Anneth Moses
#QUESTION2
# import networkx
...
###################################
    ...
    while frontier:
        current_node = frontier.pop(0) # Pop the first element in the list (queue structure)
...
def drawBFS():
    ...
    color_map = []
    # for all the nodes in the graph, assign the color green if it's part of the path , else, red
    for node in G.nodes:
        if node in bfs_path:
...
def ucs(startNode, goal):
    ...
    while frontier:
        ...
        current_node = ucs_result[-1][0] # The current node is the last node in the ucs path
        ...
        if current_node == goal:
            ...
            for node in G.neighbors(current_node):
                ...
                new_path.append((node, G[current_node][node]['weight']))
...
def drawUCS():
    ...
    color_map = []
    #assign green to all the nodes in the graph that are part of the path , otherwise, red
    for node in G.nodes:
